...
class densenet(nn.Module)
    def __init__(self, depth=40, 
        ...
        if cfg == None
            ...
            start = growthRate2
            for _ in range(3)
                cfg.append([start + growthRatei for i in range(n+1)])
                start += growthRaten
            cfg = [item for sub_list in cfg for item in sub_list]

        assert len(cfg) == 3n+3, 'length of config variable cfg should be 3n+3'

        self.inplanes = growthRate  2
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, padding=1,
                               bias=False)
        self.dense1 = self._make_denseblock(block, n, cfg[0n])
        self.trans1 = self._make_transition(compressionRate, cfg[n])
        self.dense2 = self._make_denseblock(block, n, cfg[n+12n+1])
        self.trans2 = self._make_transition(compressionRate, cfg[2n+1])
        self.dense3 = self._make_denseblock(block, n, cfg[2n+23n+2])
        ...
        if use_hooks
            ...
            for module in self.modules()
                if isinstance(module, nn.BatchNorm2d)
                    hooks.append(module.register_forward_hook(save_output_hook))
                if isinstance(module, nn.Conv2d) and flag!=0
